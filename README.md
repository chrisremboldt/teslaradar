...
| Ownship | Rolling in-memory GPS track (last 5 minutes). Chevron points at the distance-weighted circular mean of segment bearings. Average speed over the same good segments draws 5- and 30-minute range rings (speed × time). Segments under ~20 m, huge accuracy, or implausible hops are ignored so parked jitter does not spin the triangle or inflate rings. |
