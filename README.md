...
- Overlays animated RainViewer radar (coordinate-centered images on a DOM/canvas layer) on a free OpenStreetMap raster basemap (darkened in MapLibre). No Mapbox or Carto token.
...
| Radar | Client fetch of `https://api.rainviewer.com/public/weather-maps.json`. Frames animate via a DOM/canvas image overlay (not MapLibre raster tiles): `{host}{path}/{size}/{z}/{lat}/{lon}/{color}/{options}.png` with Universal Blue (scheme `2`), 512px, `z` ≤ 7. Free-tier notes (2026): past frames ~2h / 10 min, rate limit on the order of 100 req/IP/min. |
