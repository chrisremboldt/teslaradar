...
- Overlays RainViewer radar tiles on a free OpenFreeMap dark / OpenStreetMap basemap (MapLibre). No Mapbox or Carto token.
...
| Map | MapLibre GL + OpenFreeMap dark (OSM). No Mapbox or Carto token. Follow-me recenters. Default is heading-up (map rotates with compass); north-up is a toggle. Falls back to north-up if heading is missing. |

**Radar data by [RainViewer](https://www.rainviewer.com/api.html).** Basemap © OpenStreetMap contributors, [OpenFreeMap](https://openfreemap.org/).
